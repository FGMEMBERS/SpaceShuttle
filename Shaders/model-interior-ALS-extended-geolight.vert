...
uniform float interior_ambience_factor;
...
// this code is copied from default.vert
 ...
 //determine up-vector in model coordinates
   mat4 RotMatPR;
   mat4 RotMatPR_tr;
   ...
   RotMatPR_tr=transpose(RotMatPR);

   geoLightVec = (gl_ModelViewMatrix  * RotMatPR_tr * vec4 (geo_light_x, geo_light_y, geo_light_z, 0.0)).xyz;
...
// correct ambient light color and intensity by external driven term

  vec3 forced_ambience = vec3 (interior_ambience_r, interior_ambience_g, interior_ambience_b);

  light_ambient.rgb = mix(light_ambient.rgb, forced_ambience, interior_ambience_factor);
