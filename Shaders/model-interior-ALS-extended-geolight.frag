...
// this is taken from default.frag
    ...
    //disable opacity mapping till this is properly implemented
    opacity = vec4 (0.5,0.5, 0.5,1.0); 
  ...
  // geometric light
    ...
    //float geo_light_incidence = geo_ambience + (1.0- geo_ambience) * clamp(dot(N, geo_light_rel_vec),0.0, 1.0);
     ...
     float geo_light_incidence =  clamp(dot(n, geoLightVec),0.0, 1.0);

    vec3 geo_light = vec3 (geo_light_r, geo_light_g, geo_light_b) * geo_light_incidence;
